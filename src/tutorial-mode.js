...
export const T1_SPEC = deepFreeze({
  ...
  // ⚠️ 一条鱼 vs 一群鱼，设计者当时未定（「一条鱼很直观，但一群鱼又更
  // 贴合。我也不清楚」）。这里取小群 4v4 作为默认：够久到能看清吃的过程，
  // 又少到一眼数得清。要改成 1v1 就把这两个数都改成 1。
  playerCount: 4,
  referenceCount: 4,
  referenceSize: 1.5,
...
export function createTutorialConfig(spec = T1_SPEC, value = spec.slider.initial) {
  ...
  // 少一类可能出问题的地方。（曾怀疑「换成 2 个鱼群导致索引错位」是
  // 浏览器内不触发追击的原因，但改回 3 个鱼群后现象照旧，该假设已被
  // 实验否定，不要再拿它解释那个 bug。）
